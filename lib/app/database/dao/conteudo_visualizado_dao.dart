...
class ConteudoVisualizadoDAO extends AbstractDAO<ConteudoVisualizado>
    with _$ConteudoVisualizadoDAOMixin {
  ...
  Future save(ConteudoVisualizado entity) async {
    if ((entity as dynamic).id == null) {
      var row =
          await into(conteudosVisualizados).insertOnConflictUpdate(entity);
  ...
  Future<List<ConteudoVisualizado>> getParaEnvio() async {
    ...
    var query = await customSelect(sql, readsFrom: {conteudosVisualizados});
    return query
        .map<ConteudoVisualizado>(
            (row) => ConteudoVisualizado.fromData(row.data, db))
        .get();
  }

  Future<ConteudoVisualizado> registrarVisualizacao(
      int idConteudo, int idNoticia) async {
    ...
    if (conteudoVisualizado == null) {
      ...
      conteudoVisualizado.horas = conteudoVisualizado.horas +
          "," +
